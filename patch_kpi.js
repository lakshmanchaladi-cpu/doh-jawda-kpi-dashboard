...
const regex = /setImmediate\(async \(\) => \{([\s\S]*?)const results = await engine\.calculateAllKPIs/m;

const newWorkerLogic = `
        try {
          ...
          function sqlLiteral(str) { return str.replace(/'/g, "''"); }
          ...
          const insuranceCases = mappings.map(m => {
            ...
            const group = sanitizeCode(m.group_name); // Simplified for safety
          ...
          const results = await engine.calculateAllKPIs`;

code = code.replace(regex, newWorkerLogic);
