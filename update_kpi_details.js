...
// KPI_DETAILS: Complete V9 data extracted directly from PC V9 PDF
const KPI_DETAILS_OBJ = {
  PC004: {
...
// Read the existing generator and inject new KPI_DETAILS
const genPath = path.join(__dirname, 'generate_settings_js.js');
let genContent = fs.readFileSync(genPath, 'utf8');

// Build the new KPI_DETAILS string
const kpiDetailsStr = JSON.stringify(KPI_DETAILS_OBJ, null, 2)
  .replace(/"([^"]+)":/g, "'$1':")   // object keys unquoted
  .replace(/: "([^"]+)"/g, ": '$1'"); // string values single quoted

// Write a standalone KPI_DETAILS injection
const outPath = path.join(__dirname, 'public', 'js', 'kpi_details_v9.js');
fs.writeFileSync(outPath, `// Auto-generated from PC V9 PDF\nconst KPI_DETAILS_V9 = ${JSON.stringify(KPI_DETAILS_OBJ, null, 2)};\n`, 'utf8');

console.log('Written kpi_details_v9.js - now updating settings.js KPI_DETAILS...');

// Update the main settings.js directly
const settingsPath = path.join(__dirname, 'public', 'js', 'settings.js');
let settingsContent = fs.readFileSync(settingsPath, 'utf8');

// Build JS-literal KPI_DETAILS string using template format
let newKpiDetails = 'const KPI_DETAILS = {\n';
for (const [code, d] of Object.entries(KPI_DETAILS_OBJ)) {
  const exclusionsStr = d.exclusions.map(e => `      '${e.replace(/'/g, "\\'")}'`).join(',\n');
  newKpiDetails += `  '${code}': {\n`;
  newKpiDetails += `    title: '${d.title.replace(/'/g, "\\'")}',\n`;
  newKpiDetails += `    denominator: '${d.denominator.replace(/'/g, "\\'").replace(/\n/g, ' ')}',\n`;
  newKpiDetails += `    numerator: '${d.numerator.replace(/'/g, "\\'").replace(/\n/g, ' ')}',\n`;
  newKpiDetails += `    exclusions: [\n${exclusionsStr}\n    ],\n`;
  newKpiDetails += `    emr_resp: '${d.emr_resp.replace(/'/g, "\\'").replace(/\n/g, ' ')}',\n`;
  newKpiDetails += `    rcm_resp: '${d.rcm_resp.replace(/'/g, "\\'").replace(/\n/g, ' ')}'\n`;
  newKpiDetails += `  },\n`;
}
newKpiDetails += '};\n';

// Replace old KPI_DETAILS block
const kpiStart = settingsContent.indexOf('const KPI_DETAILS = {');
const kpiEnd = settingsContent.indexOf('\n};\n', kpiStart) + 4;
settingsContent = settingsContent.slice(0, kpiStart) + newKpiDetails + settingsContent.slice(kpiEnd);

fs.writeFileSync(settingsPath, settingsContent, 'utf8');
console.log('Successfully updated KPI_DETAILS in settings.js with complete V9 data.');
