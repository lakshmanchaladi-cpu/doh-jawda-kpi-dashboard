  ...
  async renderVault(container) {
    ...
    try {
      ...
      data.forEach(q => {
        ...
        const lastCalc = q.last_calculated_at ? new Date(q.last_calculated_at).toLocaleDateString() : 'Never calculated';
        
        html += `
          <div class="col-12 col-xl-6">
            <div class="card shadow-sm border-0 h-100">
              <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold"><i class="bi bi-calendar3 me-2 text-primary"></i> ${qLabel}</h5>
                <span class="badge ${badgeClass} fs-6">${statusText} (${q.match_rate}%)</span>
              </div>
              <div class="card-body">
                <div class="row text-center mb-4">
                  <div class="col">
                    <div class="text-muted small text-uppercase mb-1">EMR Records</div>
                    <div class="fs-4 fw-bold">${q.emr_count.toLocaleString()}</div>
                  </div>
                  <div class="col">
                    <div class="text-muted small text-uppercase mb-1">RCM Claims</div>
                    <div class="fs-4 fw-bold">${q.rcm_count.toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div class="d-flex justify-content-between text-muted small">
                  <span>Match Rate: ${q.match_rate}%</span>
                  <span>Last Calculated: <span id="calc-date-${q.year}-${q.quarter}">${lastCalc}</span></span>
                </div>
              </div>
              <div class="card-footer bg-light p-3 d-flex justify-content-between gap-2">
                <button class="btn btn-outline-secondary btn-sm" onclick="DataManager.downloadExceptions(${q.year}, ${q.quarter})">
                  <i class="bi bi-download"></i> Exceptions
                </button>
                <button class="btn btn-primary btn-sm flex-grow-1" id="btn-calc-${q.year}-${q.quarter}" onclick="DataManager.calculateKpis(${q.year}, ${q.quarter})">
